...
throttle=float(input("enter the initial value(pwm))"))

set_point = float(input("enter the setpoint value(rpm)"))
...
def speed_of_motor():
    ...
    #error=(set_point-speed)
    #error=((set_point-speed)*(set_point-speed)/2)**(0.5)
    ...
    correction =((prop_gain*error)+ (derivative_gain*differential_error) + (integral_gain*integral_error))/3
...
def my_call(channel):
    ...
    counter= counter+1
    #print("\n",counter)
    if ((counter%sample_count_encoder_holes)==0):
        s_count=s_count+1#checking for exact 10 counts sampling after counts
        speed_of_motor()
